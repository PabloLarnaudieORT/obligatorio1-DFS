...
export const obtenerDesafiosService = async () => {
  ...
  const resultado = await Promise.all(
    desafios.map(async (desafio) => {
      const ejercicios = await DesafioEjercicio.find({ idDesafio: desafio._id })
        .populate("idDesafio", "nombreDesafio")
        .populate("idEjercicios", "nombreEjercicio");
...
export const actualizarDesafioService = async (id, data) => {
  ...
  const desafioActualizado = await Desafio.findByIdAndUpdate(id, data, {

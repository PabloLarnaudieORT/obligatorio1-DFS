...
export const crearCategoriaZonaMusculo = async (req, res) => {
  ...
  const categoriaCreada = await crearCategoriaZonaMusculoService(
    req.body,
    userId,
  );
  res.json({
    message: "Categoría de zona muscular agregada exitosamente",
    ...categoriaCreada,
  });
};
...
export const obtenerCategoriaZonaMusculoServicePorId = async (req, res) => {
  ...
  const categoriaObtenida =
    await obtenerCategoriaZonaMusculoServicePorIdService(id, userIdFilter);
  res.json({
    message: `Categoría de zona muscular ${categoriaObtenida.id} obtenida con exito`,
...
export const actualizarCategoriaZonaMusculo = async (req, res) => {
  ...
  const categoriaActualizada = await actualizarCategoriaZonaMusculoService(
    id,
    req.body,
    userIdFilter,
  );
  res.json({
    message: `Categoría de zona muscular ${categoriaActualizada.id} actualizada exitosamente`,
...
export const eliminarCategoriaZonaMusculo = async (req, res) => {
  ...
  const categoriaEliminada = await eliminarCategoriaZonaMusculoService(
    id,
    userIdFilter,
  );
  res.json({
    message: `Categoría de zona muscular ${categoriaEliminada.id} eliminada exitosamente`,
    ...categoriaEliminada,
  });
};
